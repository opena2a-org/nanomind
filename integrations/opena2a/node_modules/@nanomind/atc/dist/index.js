...
exports.ATCIntentHandler = void 0;
const DEFAULT_REGISTRY_URL = 'https://api.oa2a.org';
class ATCIntentHandler {
    registryUrl;
    apiKey;
    cache = new Map();
    cacheTTL;
     ...
     */
    async explainTrustLevel(agentId) {
        const atc = await this.fetchATC(agentId);
        const factors = [];
        ...
        const summary = atc
            ? `Your agent ${agentId} is trust level ${currentLevel} because:\n` +
                factors.map(f => {
                    if (f.status === 'present')
                        return `  Present: ${f.name} (${f.impact})`;
...
exports.ATCIntentHandler = ATCIntentHandler;
exports.default = ATCIntentHandler;
