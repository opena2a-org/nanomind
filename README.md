...
|-------|-----------|--------------|----------|--------|
| [nanomind-security-classifier](https://huggingface.co/opena2a/nanomind-security-classifier) | Public | Mamba TME (10 classes) | Recall 100%, Precision 79.6%, F1 88.7%, Benign FPR 9.1% (oracle-verified, TME v5 2026-04-15) | Production (v0.5.0 — terminal classifier release) |
...
>
> **v3.0.0 — FP-suppression on benign security code (corrected 2026-06-03).** The analyst can over-classify legitimate JWT validators, RBAC middleware, parameterized queries, rate limiters, and OAuth/crypto code as threats. On a repaired 82-sample gate of *real* security-library source it correctly clears **77%** as benign — the originally documented 57% / 43%-FP figure was measured on a gate later found to be 41% placeholder filler, which understated reliability. Spot-checking findings on dedicated security libraries is still advisable (~1 in 4 may be over-flagged). A corpus retrain (v3.1) was a no-go; the scoped fix is a benign-security pre-pass, not corpus expansion. See model card §Known Limitations §2.
>
