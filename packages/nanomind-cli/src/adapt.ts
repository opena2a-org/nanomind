...
export class SkillAdapter {
  ...
  private inferredLevel: SkillLevel = 'intermediate';
   ...
   */
  private infer(): SkillLevel {
    if (this.history.length < 3) return 'intermediate';
    ...
    for (const entry of recent) {
      ...
      if (EXPERT_SIGNALS.some(s => cmd.includes(s))) expertScore++;
      if (BEGINNER_SIGNALS.some(s => cmd.includes(s))) beginnerScore++;
      if (entry.success) expertScore += 0.5;
    }

    const ratio = expertScore / (beginnerScore + 1);
    if (ratio > 3) return 'expert';
